...
namespace Vars
{
	NAMESPACE_BEGIN(Menu)
		...
		CVar(Scale, "Scale", 1.f, NOBIND | SLIDER_MIN | SLIDER_PRECISION | SLIDER_NOAUTOUPDATE, 0.75f, 2.f, 0.05f);
	...
	SUBNAMESPACE_END(FONT_FLAGS);

	SUBNAMESPACE_BEGIN(FONT_TAGS);
	...
	NAMESPACE_BEGIN(Colors)
		...
		CVar(UberBarHigh, "Ubercharged Color", Color_t(75, 175, 255, 255), VISUAL);
		CVar(UberBarLow, "Ubercharged Color", Color_t(255, 0, 0, 255), VISUAL);
		...
		CVar(ElementsColor, "Text elements color", Color_t(255, 255, 255, 255), VISUAL);
		CVar(ElementsOutline, "Text elements outline", Color_t(0, 0, 0, 255), VISUAL);
	...
	NAMESPACE_BEGIN(Aimbot)
		SUBNAMESPACE_BEGIN(General, Aimbot)
			CVarEnum(AimType, "Aimbot type", 0, NONE, nullptr,
		...
		CVar(AutoShoot, "Automatic fire", true);
		CVar(FOVCircle, "Aimbot overlay", true, VISUAL);
		CVar(NoSpread, "Remove spread", false);
		CVar(AlwaysMelee, "Always melee", false);
		CVar(OnlyDtAimbot, "DT Aimbot only", false);
		...
		SUBNAMESPACE_BEGIN(Projectile)
			...
			CVarEnum(AutoDetonate, "Auto detonate", 0b00, DROPDOWN_MULTI, "Off",
				VA_LIST("Stickies", "Flares", "##Divider", "Prevent self damage"),
				Stickies = 1 << 0, Flares = 1 << 1, PreventSelfDamage = 1 << 3);
			...
			CVar(MaxSimulationTime, "Max simulation time", 2.f, SLIDER_MIN | SLIDER_PRECISION, 0.1f, 5.f, 0.25f, "%gs");
			CVar(HitChance, "Hit chance", 0.f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 100.f, 5.f, "%g%%");
			CVar(AutodetRadius, "Autodet radius", 90.f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 100.f, 10.f, "%g%%");
			CVar(SplashRadius, "Splash radius", 90.f, NONE, 0.f, 100.f, 5.f, "%g%%");
			CVar(AutoRelease, "Auto release", 0.f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 100.f, 5.f, "%g%%");
			CVarEnum(SplashMode, "Splashbot version", 0, NONE, nullptr, VA_LIST("Amalgam", "Custom"), Multi, Single);
			CVarEnum(RocketSplashMode, "Rocket splash mode", 0, NONE, nullptr, VA_LIST("Regular", "Special light", "Special heavy"), Regular, SpecialLight, SpecialHeavy);
			CVar(SplashPointsDirect, "Splash points", 600, NONE, 0, 2000, 10);
			CVar(SplashPointsArc, "Splash points arc", 400, SLIDER_CLAMP | SLIDER_PRECISION, 0, 2000, 10);
			CVar(SplashPointDensity, "Point density", 100, SLIDER_CLAMP | SLIDER_PRECISION, 1, 100, 1);
			CVar(EdgeSamples, "Edge samples", 8, SLIDER_CLAMP | SLIDER_PRECISION, 2, 16, 1);
			CVar(MinEdgePoints, "Min Edge points", 4, SLIDER_CLAMP | SLIDER_PRECISION, 4, 24, 1);
			CVar(MaxEdgePoints, "Max Edge points", 16, SLIDER_CLAMP | SLIDER_PRECISION, 10, 50, 1);
			CVar(EdgeSearchRadius, "Edge search radius", 64, SLIDER_CLAMP | SLIDER_PRECISION, 0, 128, 1);
			CVar(EdgeDetection, "Edge detection", false);
			CVar(UseSplashPointDensity, "Use density", false);
			CVar(UsePointSplash, "Test splashbot", false);
			CVar(UseEyeTrace, "Use eye trace", false);
			CVar(Testsplash, "Delay splash", false);
			CVar(UseHighArc, "Arc", false);


			//CVarEnum(SplashAdditions, VA_LIST("Splash additions", "nigger"), 0b001111, DROPDOWN_MULTI, nullptr,
			//	VA_LIST("Smart splash", "Test splashbot", "Eye trace"), delay = 1 << 0, UsePointSplash = 1 << 1, UsePointSplashEye = 1 << 2);
			...
			CVar(SplashUpdateRate, "Splash Rate (RPF)", 5, NONE, 1, 25);
			CVar(TraceVisibility, "Trace visibility", 100, NONE, 85, 100);
			...
			CVar(splashrayoffset, "splashrayoffset", 99, NOSAVE | DEBUGVAR | SLIDER_MIN, 0, 100, 1);
			CVar(splashrayinside, "splashrayinside", 10, NOSAVE | DEBUGVAR | SLIDER_MIN, 0, 100, 1);
		SUBNAMESPACE_END(Projectile);

		SUBNAMESPACE_BEGIN(DebugProj)
			CVar(debugsplash, "Draw splash", false);
			CVar(airfix, "Air fix", false);
			CVar(TestEdge, "Experimental edge detection", false);
			CVarEnum(EdgeSplashPriority, "Edge splash mode", 0, NONE, nullptr, VA_LIST("Off", "Prefer", "Force"), Off, Prefer, Force);
			CVar(bleedstrictness, "Bleed loose", 90, NOSAVE | DEBUGVAR | SLIDER_MIN, 1, 100, 1);
			CVar(explosionoffset, "Explosion offset", 0, NOSAVE | DEBUGVAR | SLIDER_MIN, 0, 5, 1);
			CVar(debuglogs, "Debug splash", false);
			CVar(debuglogs2, "Debug prefer", false);
			CVarEnum(debug_opt, VA_LIST("Debug options", "debug splashbot options"), 0b000111, DROPDOWN_MULTI, nullptr,
				VA_LIST("Bleed validation", "Seamshot checks", "Side checks", "Final decision", "Fast LOS checks"),
				bleed = 1 << 0, seam = 1 << 1, side = 1 << 2, final = 1 << 3, los = 1 << 4);
			CVar(los_checks, "LOS Checks", 6, NOSAVE | DEBUGVAR | SLIDER_MIN, 1, 15, 1);
			CVar(max_points, "Splash cap", 200, NOSAVE | DEBUGVAR | SLIDER_MIN, 50, 500, 5);

			CVar(fick_freq, "Flick frequency", 55, NONE, 10, 100, 1);
			CVar(flick_dist, "Flick distance", 55, NONE, 10, 500, 10);
			CVar(strafe_strictness, "Strafe strictness", 90, NONE, 10, 360, 5);

		SUBNAMESPACE_END(DebugProj);
		...
		SUBNAMESPACE_BEGIN(Melee)
			...
			CVar(SwingOffset, "Swing offset", -1, SLIDER_MIN | SLIDER_PRECISION, -1, 1);
	...
	NAMESPACE_BEGIN(Backtrack)
		...
		CVar(PreferOnShot, "Prefer shot records", false);
	...
	NAMESPACE_BEGIN(Visuals)
		...
		SUBNAMESPACE_BEGIN(Effects)
            ...
            CVar(CritGlow, "Crit glow", false, VISUAL);
		...
		SUBNAMESPACE_BEGIN(World)
			...
			CVar(matFullbright, "mat_fullbright", false, VISUAL);
	...
	NAMESPACE_BEGIN(Misc)
		SUBNAMESPACE_BEGIN(Movement)
			...
			CVar(AutoStrafeSmoothing, "Strafe smoothing", 50.f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 100.f, 5.f);
