...
void CVisuals::ProjectileTrace(CTFPlayer* pPlayer, CTFWeaponBase* pWeapon, const bool bQuick)
{
	...
	Vec3 vAngles = (G::CurrentUserCmd && !bQuick) ? G::CurrentUserCmd->viewangles : I::EngineClient->GetViewAngles();
	int iFlags = ProjSimEnum::Trace | ProjSimEnum::InitCheck;

	if (bQuick) {
		iFlags |= ProjSimEnum::Quick;
	}

	if (bQuick && F::Spectate.m_iTarget != -1)
	{
		auto pTarget = I::ClientEntityList->GetClientEntity(I::EngineClient->GetPlayerForUserID(F::Spectate.m_iTarget));
		if (pTarget)
			pPlayer = pTarget->As<CTFPlayer>();
	}
...
void CVisuals::CreateMove(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	if (Vars::Visuals::World::matFullbright.Value)
	{
		I::EngineClient->ClientCmd_Unrestricted("mat_fullbright 1");
	}
	else {
		I::EngineClient->ClientCmd_Unrestricted("mat_fullbright 0");
	}
