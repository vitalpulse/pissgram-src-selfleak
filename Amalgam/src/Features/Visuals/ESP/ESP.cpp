...
static inline void StorePlayer(CTFPlayer* pPlayer, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, PlayerCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & ESPEnum::HealthBar)
	{
			...
			pPlayer->InCond(TF_COND_INVULNERABLE_CARD_EFFECT))
		{
			...
			tCache.m_vBars.emplace_back(ALIGN_LEFT, smoothHealth, tColor, Vars::Colors::UberColor.Value, (pGroup->m_iESPOptions & ESPOptionsEnum::DynamicBars) ? true : false);
		}
		else
		{
			tCache.m_vBars.emplace_back(ALIGN_LEFT, (smoothHealth > 0.99f && smoothHealth < 1.0f) ? 1.0f : smoothHealth, tColor, Vars::Colors::OverhealColor.Value, (pGroup->m_iESPOptions & ESPOptionsEnum::DynamicBars) ? true : false);
	...
	if (pGroup->m_iESP & (ESPEnum::UberBar | ESPEnum::UberText) && iClassNum == TF_CLASS_MEDIC)
	{
		...
		if (pMediGun && pMediGun->GetClassID() == ETFClassID::CWeaponMedigun)
		{
			...
			if (pGroup->m_iESP & ESPEnum::UberBar)
				tCache.m_vBars.emplace_back(ALIGN_BOTTOM, flUber, uberColor, Color_t(), (pGroup->m_iESPOptions & ESPOptionsEnum::DynamicBars) ? true : false);
...
static inline void StoreBuilding(CBaseObject* pBuilding, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, BuildingCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & ESPEnum::HealthBar)
	{
		...
		if (smoothHealth > 0.5f)
		{
			float factor = (smoothHealth - 0.5f) / 0.5f;
			tColor = Vars::Colors::MidHPColor.Value.Lerp(Vars::Colors::FullHPColor.Value, factor);
		}
		else
		{
			float factor = smoothHealth / 0.5f;
			tColor = Vars::Colors::LowHPColor.Value.Lerp(Vars::Colors::MidHPColor.Value, factor);
		}
		...
		tCache.m_vBars.emplace_back(ALIGN_LEFT, smoothHealth, tColor, Vars::Colors::IndicatorMisc.Value, (pGroup->m_iESPOptions & ESPOptionsEnum::DynamicBars) ? true : false);
	}
	tCache.m_flSmoothHealth = smoothHealth;
	...
	if (pGroup->m_iESP & (ESPEnum::AmmoBars | ESPEnum::AmmoText) && pBuilding->IsSentrygun() && !pBuilding->m_bBuilding())
	{
		...
		{
			tCache.m_vBars.emplace_back(ALIGN_BOTTOM, float(iShells) / iMaxShells, Vars::Menu::Theme::Inactive.Value, Color_t(), (pGroup->m_iESPOptions & ESPOptionsEnum::DynamicBars) ? true : false);
			if (iMaxRockets)
				tCache.m_vBars.emplace_back(ALIGN_BOTTOM, float(iRockets) / iMaxRockets, Vars::Menu::Theme::Inactive.Value, Color_t(), (pGroup->m_iESPOptions & ESPOptionsEnum::DynamicBars) ? true : false);
		}
