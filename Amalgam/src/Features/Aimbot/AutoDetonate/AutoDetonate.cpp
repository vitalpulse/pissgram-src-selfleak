...
static inline bool CheckEntities(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd, CBaseEntity* pProjectile, float flRadius, Vec3 vOrigin)
{
		...
		sphere.NextEntity())
	{
		...
		if (Vars::Aimbot::Projectile::AutoDetonate.Value & Vars::Aimbot::General::IgnoreEnum::Invisible && pEntity->IsPlayer() && pEntity->As<CTFPlayer>()->IsInvisible(Vars::Aimbot::General::IgnoreInvisible.Value / 100.f))
			continue;
