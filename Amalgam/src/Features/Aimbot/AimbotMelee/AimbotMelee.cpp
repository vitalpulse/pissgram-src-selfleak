...
int CAimbotMelee::CanHit(Target_t& tTarget, CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	for (auto pRecord : vRecords)
	{
		...
		std::vector<Vec3> vTestPoints;

		Vec3 vDiff = { 0, 0, std::clamp(m_vEyePos.z - pRecord->m_vOrigin.z, pRecord->m_vMins.z, pRecord->m_vMaxs.z) };
		vTestPoints.push_back(pRecord->m_vOrigin + vDiff);
		vTestPoints.push_back(pRecord->m_vOrigin + Vec3(0, 0, pRecord->m_vMaxs.z));
		vTestPoints.push_back(pRecord->m_vOrigin + Vec3(0, 0, pRecord->m_vMins.z));
		vTestPoints.push_back(pRecord->m_vOrigin + Vec3(0, 0, (pRecord->m_vMins.z + pRecord->m_vMaxs.z) / 2.0f));
		vTestPoints.push_back(pRecord->m_vOrigin + Vec3(0, 0, pRecord->m_vMaxs.z * 0.75f));
		vTestPoints.push_back(pRecord->m_vOrigin + Vec3(0, 0, pRecord->m_vMaxs.z * 0.25f));

		bool bHitFound = false;
		Vec3 vBestPos;
		float flBestFOV = 180.0f;

		for (const auto& vTestPos : vTestPoints)
		{
			Vec3 vAngleTo = Math::CalcAngle(m_vEyePos, vTestPos);
			float flFOV = Math::CalcFov(I::EngineClient->GetViewAngles(), vAngleTo);
			...
			if (bCanHit)
			{
				...
				{
					if (!CanBackstab(tTarget.m_pEntity, pLocal, vAngleTo))
						continue;
				}
		...
		else
		{
			...
			{
				Vec3 vDiff = { 0, 0, std::clamp(m_vEyePos.z - pRecord->m_vOrigin.z, pRecord->m_vMins.z, pRecord->m_vMaxs.z) };
				Vec3 vPos = pRecord->m_vOrigin + vDiff;
				auto vAngle = Math::CalcAngle(m_vEyePos, vPos);
...
void CAimbotMelee::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	if (!Vars::Aimbot::General::AimType.Value && !Vars::Aimbot::General::AlwaysMelee.Value)
		return;

	if (!F::AimbotGlobal.ShouldAim() && pWeapon->m_flSmackTime() < 0.f)
		return;
