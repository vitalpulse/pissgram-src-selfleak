...
void CMisc::AutoStrafe(CTFPlayer* pLocal, CUserCmd* pCmd)
{
	...
	case Vars::Misc::Movement::AutoStrafeEnum::Directional:
	{
		...
		vForward.Normalize2D();
		vRight.Normalize2D();

		Vec3 vWishDir = Math::VectorAngles({
			vForward.x * flForward + vRight.x * flSide,
			vForward.y * flForward + vRight.y * flSide,
			0.f
			});

		Vec3 vCurDir = Math::VectorAngles(pLocal->m_vecVelocity());
		float flDirDelta = Math::NormalizeAngle(vWishDir.y - vCurDir.y);
		float flSpeed = pLocal->m_vecVelocity().Length2D();
		static auto sv_airaccelerate = H::ConVars.FindVar("sv_airaccelerate");
		float flAirAccel = sv_airaccelerate ? sv_airaccelerate->GetFloat() : 10.f;
		float flOptimalAngle = 0.f;
		if (flSpeed > 50.f)
		{
			float flMaxSpeed = pLocal->m_flMaxspeed();
			float flSpeedRatio = std::min(flSpeed / flMaxSpeed, 2.0f);

			flOptimalAngle = 15.f + (5.f * flSpeedRatio);
		}


		float flSmoothing = Vars::Misc::Movement::AutoStrafeSmoothing.Value / 100.f;
		float flTurnScale = 0.9f + (0.1f * flSmoothing);
		float flMaxDelta = 180.f - (160.f * flSmoothing);

		if (flSmoothing > 0.5f && flSpeed > 100.f)
		{
			float flOptimalDelta = Math::NormalizeAngle(flDirDelta - (flDirDelta > 0.f ? flOptimalAngle : -flOptimalAngle));
			float flBlend = (flSmoothing - 0.5f) * 2.f;
			flDirDelta = flDirDelta * (1.f - flBlend) + flOptimalDelta * flBlend;
		}

		if (fabsf(flDirDelta) > flMaxDelta)
			break;
