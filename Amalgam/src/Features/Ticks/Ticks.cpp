...
static constexpr int ClampValue(int value, int min, int max)
{
	if (value < min)
		return min;
	if (value > max)
...
void CTicks::Draw(CTFPlayer* pLocal) {
	...
	if (!m_bSpeedhack)
	{
		// Get the full display maximum (unaffected by recharge limit)
		static auto sv_maxusrcmdprocessticks = H::ConVars.FindVar("sv_maxusrcmdprocessticks");
		...
		if (iTicks > 0)
		{
			...
			// Pixel-perfect rounding logic retained
			...
			for (int iStart = 0; iStart < iLoopLimit; iStart += iSegmentWidth)
			{
				...
				// Pixel-perfect drawing X coordinate retained
				H::Draw.FillRect(iPosX + iStart, iPosY,
					iWidth, iSizeY,
					tBlend);
			}
		...
		H::Draw.StringOutlined(fFont, dtPos.x, dtPos.y + 2, Vars::Menu::Theme::IndicatorsColor.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOP, std::format("Speedhack x{}", Vars::Speedhack::Amount.Value).c_str());
}