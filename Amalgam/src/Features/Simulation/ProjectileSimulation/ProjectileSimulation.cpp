...
bool CProjectileSimulation::GetInfoMain(CTFPlayer* pPlayer, CTFWeaponBase* pWeapon, Vec3 vAngles, ProjectileInfo& tProjInfo, int iFlags, float flAutoCharge)
{
	...
	case TF_WEAPON_CANNON:
	{
		...
		if (!bCannon && pWeapon->m_iItemDefinitionIndex() == Demoman_m_TheLochnLoad && flSpeed != 3000.f)
		{
			constexpr float flLochSpeedMult = 1.25f;
			float flWeaponSpeed = 0.f;

			if (auto pWeaponInfo = pWeapon->GetWeaponInfo())
				flWeaponSpeed = pWeaponInfo->GetWeaponData(0).m_flProjectileSpeed;

			if (flWeaponSpeed > flSpeed)
				flSpeed = flWeaponSpeed;
			else if (flSpeed <= 1300.f) // guard against double-scaling if attributes already applied
				flSpeed *= flLochSpeedMult;
		}
