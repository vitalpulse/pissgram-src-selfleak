...
static inline bool GetYawDifference(MoveData& tRecord1, MoveData& tRecord2, bool bStart, float* pYaw, float flStraightFuzzyValue, int iMaxChanges = 0, int iMaxChangeTime = 0, float flMaxSpeed = 0.f)
{
	...
	const Vector& vecVel1 = tRecord2.m_vVelocity;
	const Vector& vecVel2 = tRecord1.m_vVelocity;

	float lenSq1 = vecVel1.x * vecVel1.x + vecVel1.y * vecVel1.y;
	float lenSq2 = vecVel2.x * vecVel2.x + vecVel2.y * vecVel2.y;

	if (lenSq1 <= FLT_EPSILON || lenSq2 <= FLT_EPSILON)
		return false;

	float len1 = std::sqrt(lenSq1);
	float len2 = std::sqrt(lenSq2);

	float dot = vecVel1.x * vecVel2.x + vecVel1.y * vecVel2.y;
	float cross = vecVel1.x * vecVel2.y - vecVel1.y * vecVel2.x;

	float invLenProduct = 1.0f / (len1 * len2);
	float flStrafeDelta = std::atan2(cross * invLenProduct, dot * invLenProduct);

	const float flTimeDiff = tRecord1.m_flSimTime - tRecord2.m_flSimTime;
	const int iTicks = std::max(TIME_TO_TICKS(flTimeDiff), 1);

	float yaw = RAD2DEG(flStrafeDelta);


	if (flMaxSpeed > 0.f && tRecord1.m_iMode != 1)
	{
		yaw *= std::clamp(len1 / flMaxSpeed, 0.f, 1.f);
	}

	if ((Vars::Aimbot::Projectile::MovesimFrictionFlags.Value & Vars::Aimbot::Projectile::MovesimFrictionFlagsEnum::CalculateIncrease) && tRecord1.m_iMode == 1)
	{
		yaw /= GetFrictionScale(len1, yaw, vecVel1.z + GetGravity() * TICK_INTERVAL, 0.f, 60.f);
	}
	...
	*pYaw = yaw;
	...
	if (bStart)
	{
		iChanges = 0, iStart = TIME_TO_TICKS(tRecord1.m_flSimTime);
	...
	else
	{
		if ((bChanged || bStraight) && ++iChanges > iMaxChanges)
			return false;
		return iChanges && iStart - TIME_TO_TICKS(tRecord2.m_flSimTime) > iMaxChangeTime ? false : true;
	}
}
...
void CMovementSimulation::GetAverageYaw(MoveStorage& tStorage, int iSamples)
{
	...
	tStorage.on_ground = tStorage.m_pPlayer->IsOnGround();
