...
static inline void LoadMain(BaseVar*& pBase, boost::property_tree::ptree& tTree)
{
	...
	//	SDK::Output("Pissgram", std::format("{} not found", pVar->m_sName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
}
	...
	try
	{
...

		{
			...
			for (int iID = 0; iID < F::Groups.m_vGroups.size(); iID++)
			{
				...
				SaveJson(tChild, "OffscreenArrowsMaxSize", tGroup.m_flOffscreenArrowsMaxSize);
				SaveJson(tChild, "OffscreenArrowsMinSize", tGroup.m_flOffscreenArrowsMinSize);
				SaveJson(tChild, "OffscreenArrowsFlickerDistance", tGroup.m_flOffscreenArrowsFlickerDistance);
				...
				SaveJson(tChild, "BacktrackGlowColor", tGroup.m_tBacktrackGlowColor);
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Config {} saved", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Save config failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
		return false;
	}
	...
	try
	{
		...
		if (auto tSub = tRead.get_child_optional("Groups"))
		{
			for (auto& [_, tChild] : *tSub)
			{
				...
				LoadJson(tChild, "BacktrackGlowModes", tGroup.m_iBacktrackGlowDraw);
				...
				LoadJson(tChild, "OffscreenArrowsMaxSize", tGroup.m_flOffscreenArrowsMaxSize);
				LoadJson(tChild, "OffscreenArrowsMinSize", tGroup.m_flOffscreenArrowsMinSize);
				LoadJson(tChild, "OffscreenArrowsFlickerDistance", tGroup.m_flOffscreenArrowsFlickerDistance);
				...
				LoadJson(tChild, "BacktrackGlowColor", tGroup.m_tBacktrackGlowColor);
		...
		m_sCurrentConfig = sConfigName; m_sCurrentVisuals = "";
		if (bNotify)
			SDK::Output("Pissgram", std::format("Config {} loaded", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Load config failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
		return false;
	}
	...
	try
	{
...

		{
			...
			for (int iID = 0; iID < F::Groups.m_vGroups.size(); iID++)
			{
				...
				SaveJson(tChild, "Projectiles", tGroup.m_iProjectiles);
				SaveJson(tChild, "ESP", tGroup.m_iESP);
				SaveJson(tChild, "ESPOptons", tGroup.m_iESPOptions);
				SaveJson(tChild, "Chams", tGroup.m_tChams);
				SaveJson(tChild, "Glow", tGroup.m_tGlow);
				SaveJson(tChild, "Backtrack", tGroup.m_bBacktrack);
				SaveJson(tChild, "BacktrackDraw", tGroup.m_iBacktrackDraw);
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Visual config {} saved", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Save visuals failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
		return false;
	}
	return true;
}
...
bool CConfigs::LoadVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Visual config {} loaded", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Load visuals failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
		return false;
	}
	return true;
}
...
void CConfigs::DeleteConfig(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Config {} deleted", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Remove config failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
	}
}
...
void CConfigs::ResetConfig(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Config {} reset", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Reset config failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
	}
}
...
void CConfigs::DeleteVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Visual config {} deleted", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Remove visuals failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
	}
}
...
void CConfigs::ResetVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Pissgram", std::format("Visual config {} reset", sConfigName).c_str(), Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG | OUTPUT_TOAST | OUTPUT_MENU);
	}
	catch (...)
	{
		SDK::Output("Pissgram", "Reset visuals failed", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_DEBUG);
	}
}