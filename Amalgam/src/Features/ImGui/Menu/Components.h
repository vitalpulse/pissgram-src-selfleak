	...
	inline bool Section(const char* sLabel, float flPaddingMod = 0.f, float flMinHeight = 28.f, bool bForceHeight = false, uint32_t uHash = 0)
	{
		...
		if (bReturn)
		{
			...
			if (sLabel[0] != '#')
			{
				...
				// Draw gradient for header (only 6 parameters!)
				...
				// Content background - ensure minimum visibility
			...
			{
				// Simple background for hidden sections - ensure minimum visibility
// Simple background for hidden sections - maintain relative visibility
	...
	{	// WOW fuck this
		...
		ImVec4 accent4 = ColorConvertU32ToFloat4(F::Render.Accent);
		float h, s, v;
		ImGui::ColorConvertRGBtoHSV(accent4.x, accent4.y, accent4.z, h, s, v);

		ImVec4 mutedAccentVec;
		ImGui::ColorConvertHSVtoRGB(h, s * 0.4f, v, mutedAccentVec.x, mutedAccentVec.y, mutedAccentVec.z);
		mutedAccentVec.w = 1.0f;

		ImU32 hoveredTextColor = GetColorU32(mutedAccentVec);
		...
		for (size_t i = bReverse ? vEntries.size() - 1 : 0; i < vEntries.size(); (bReverse ? i-- : i++))
		{
			for (size_t j = bReverse ? vEntries[i].size() - 1 : 0; j < vEntries[i].size(); (bReverse ? j-- : j++))
			{
				...
				ImVec4 colTarget = (iTabState == 2) ? ImColor(F::Render.Accent).Value : mutedAccentVec;
	...
	inline bool FButton(const char* sLabel, int iFlags = FButtonEnum::None, ImVec2 vSize = { 0, 30 }, int iSizeOffset = 0, ImFont* pFont = nullptr, bool* pHovered = nullptr)
	{
		...
		static std::unordered_map<std::string, float> mButtonHover = {};
		std::string sKey = std::format("{}_{}", sLabel, vOriginalPos.x);
		if (!mButtonHover.contains(sKey))
			mButtonHover[sKey] = 0.f;

		float& flHover = mButtonHover[sKey];
		float flTarget = bWithin ? 1.f : 0.f;
		flHover += (flTarget - flHover) * 0.2f;
		...
		if (flHover > 0.01f)
		{
			ImColor tHoverColor = tColor;
			tHoverColor.Value.w *= bClicked ? 0.1f * flHover : 0.05f * flHover;
			flInset = H::Draw.Scale();
	...
	inline bool FSlider(const char* sLabel, float* pVar1, float* pVar2, float flMin, float flMax, float flStep = 1.f, const char* fmt = "%g", int iFlags = FSliderEnum::None, bool* pHovered = nullptr)
	{
...
#else
                ImVec2 vMins = { H::Draw.Scale(6), vSize.y - H::Draw.Scale(8) }, vMaxs = { vSize.x - H::Draw.Scale(6), vSize.y - H::Draw.Scale(4) };
#endif
                ImVec4 tAccentBase = F::Render.Accent.Value;
                float flHue{}, flSaturation{}, flValue{};
                ImGui::ColorConvertRGBtoHSV(tAccentBase.x, tAccentBase.y, tAccentBase.z, flHue, flSaturation, flValue);
                float flAlpha = GetStyle().Alpha;
                auto BuildAccent = [&](float flSatScale, float flAlphaScale)
                {
                        ImVec4 tColor;
                        ImGui::ColorConvertHSVtoRGB(flHue, std::clamp(flSaturation * flSatScale, 0.f, 1.f), flValue, tColor.x, tColor.y, tColor.z);
                        tColor.w = flAlpha * flAlphaScale;
                        return ImColor(tColor);
                };

                ImColor tAccent = BuildAccent(1.f, 1.f), tTransparent = BuildAccent(1.f, 0.1f);

                auto AddGradientRect = [&](ImVec2 vMin, ImVec2 vMax, ImColor tLeft, ImColor tRight)
                {
                        pDrawList->AddRectFilledMultiColor({ vDrawPos.x + vMin.x, vDrawPos.y + vMin.y }, { vDrawPos.x + vMax.x, vDrawPos.y + vMax.y }, tLeft, tRight, tRight, tLeft);
                };
		...
		ImVec2 vMouse = GetMousePos();
                ...
                float flTrackHeight = vMaxs.y - vMins.y;
                float flDotY = vMins.y + flTrackHeight / 2.f;
                float flDotSize = H::Draw.Scale(10);
                if (pVar2)
                {
                        ...
                        float flEmptySatLower = 0.2f + 0.8f * flLowerPerc;
                        float flEmptySatUpper = 0.2f + 0.8f * flUpperPerc;
                        ImColor tWashedLower = BuildAccent(flEmptySatLower, 0.35f), tMutedLower = BuildAccent(flEmptySatLower * 0.85f, 0.6f);
                        ImColor tWashedUpper = BuildAccent(flEmptySatUpper, 0.35f), tMutedUpper = BuildAccent(flEmptySatUpper * 0.85f, 0.6f);
                        ImColor tWashedFilled = BuildAccent(1.f, 0.35f);

                        AddGradientRect(vMins, { flLowerPos, vMaxs.y }, tWashedLower, tMutedLower);
                        AddGradientRect({ flLowerPos, vMins.y }, { flUpperPos, vMaxs.y }, BuildAccent(1.f, 0.75f), BuildAccent(0.9f, 0.75f));
                        AddGradientRect({ flUpperPos, vMins.y }, vMaxs, tWashedUpper, tMutedUpper);

                        AddSteppedRect(vDrawPos, vMins, vMaxs, vMins, { flLowerPos, vMaxs.y }, flMin, flMax, flStep, tWashedLower, tMutedLower, H::Draw.Scale(3));
                        AddSteppedRect(vDrawPos, vMins, vMaxs, { flLowerPos, vMins.y }, { flUpperPos, vMaxs.y }, flMin, flMax, flStep, tAccent, tWashedFilled, H::Draw.Scale(3));
                        AddSteppedRect(vDrawPos, vMins, vMaxs, { flUpperPos, vMins.y }, vMaxs, flMin, flMax, flStep, tWashedUpper, tMutedUpper, H::Draw.Scale(3));

                        pDrawList->AddRectFilled({ vDrawPos.x + flLowerPos - flDotSize / 2.f, vDrawPos.y + flDotY - flDotSize / 2.f }, { vDrawPos.x + flLowerPos + flDotSize / 2.f, vDrawPos.y + flDotY + flDotSize / 2.f }, tAccent, H::Draw.Scale(2));
                        pDrawList->AddRectFilled({ vDrawPos.x + flUpperPos - flDotSize / 2.f, vDrawPos.y + flDotY - flDotSize / 2.f }, { vDrawPos.x + flUpperPos + flDotSize / 2.f, vDrawPos.y + flDotY + flDotSize / 2.f }, tAccent, H::Draw.Scale(2));
			...
			if (!Disabled)
			{
                                if (bWithin && !mActiveMap[uHash])
                                {
                                        ...
                                        pDrawList->AddRectFilled({ vDrawPos.x + (iVar == 1 ? flLowerPos : flUpperPos) - flDotSize, vDrawPos.y + flDotY - flDotSize }, { vDrawPos.x + (iVar == 1 ? flLowerPos : flUpperPos) + flDotSize, vDrawPos.y + flDotY + flDotSize }, tTransparent, H::Draw.Scale(2));
                                }
                                else if (mActiveMap[uHash] && IsMouseDown(ImGuiMouseButton_Left))
                                {
                                        ...
                                        pDrawList->AddRectFilled({ vDrawPos.x + (bVar1 ? flLowerPos : flUpperPos) - flDotSize * 1.2f, vDrawPos.y + flDotY - flDotSize * 1.2f }, { vDrawPos.x + (bVar1 ? flLowerPos : flUpperPos) + flDotSize * 1.2f, vDrawPos.y + flDotY + flDotSize * 1.2f }, tTransparent, H::Draw.Scale(2));
                                }
				else
		...
		else
		{
                        ...
                        float flEmptySat = 0.2f + 0.8f * flPercent;
                        ImColor tWashedFilled = BuildAccent(1.f, 0.35f);
                        ImColor tWashedEmpty = BuildAccent(flEmptySat, 0.35f);
                        ImColor tMutedEmpty = BuildAccent(flEmptySat * 0.85f, 0.6f);

                        AddGradientRect(vMins, { flPos, vMaxs.y }, BuildAccent(1.f, 0.75f), BuildAccent(0.9f, 0.75f));
                        AddGradientRect({ flPos, vMins.y }, vMaxs, tWashedEmpty, tMutedEmpty);

                        AddSteppedRect(vDrawPos, vMins, vMaxs, vMins, { flPos, vMaxs.y }, flMin, flMax, flStep, tAccent, tWashedFilled, H::Draw.Scale(1));
                        AddSteppedRect(vDrawPos, vMins, vMaxs, { flPos, vMins.y }, vMaxs, flMin, flMax, flStep, tWashedEmpty, tMutedEmpty, H::Draw.Scale(1));
                        pDrawList->AddRectFilled({ vDrawPos.x + flPos - flDotSize / 2.f, vDrawPos.y + flDotY - flDotSize / 2.f }, { vDrawPos.x + flPos + flDotSize / 2.f, vDrawPos.y + flDotY + flDotSize / 2.f }, tAccent, H::Draw.Scale(2));
			...
			if (!Disabled)
			{
                                if (bWithin && !mActiveMap[uHash])
                                {
                                        if (IsMouseClicked(ImGuiMouseButton_Left))
                                                mActiveMap[uHash] = 1;
                                        pDrawList->AddRectFilled({ vDrawPos.x + flPos - flDotSize, vDrawPos.y + flDotY - flDotSize }, { vDrawPos.x + flPos + flDotSize, vDrawPos.y + flDotY + flDotSize }, tTransparent, H::Draw.Scale(2));
                                }
                                else if (mActiveMap[uHash] && IsMouseDown(ImGuiMouseButton_Left))
                                {
                                        flSVar1 = flMin + (flMax - flMin) * flMousePerc;
                                        flSVar1 = std::clamp(flSVar1 - fnmodf(flSVar1, flStep), flMin, flMax);
                                        pDrawList->AddRectFilled({ vDrawPos.x + flPos - flDotSize * 1.2f, vDrawPos.y + flDotY - flDotSize * 1.2f }, { vDrawPos.x + flPos + flDotSize * 1.2f, vDrawPos.y + flDotY + flDotSize * 1.2f }, tTransparent, H::Draw.Scale(2));
                                }
				else
					mActiveMap[uHash] = false;
		...
		PopStyleColor();
                ...
                Button("##", { vMaxs.x - vMins.x + 12, flTrackHeight + H::Draw.Scale(12) }); // don't drag it around
		SetCursorPos(vOriginalPos);
		AddRowSize(vOriginalPos, vSize);
		DebugDummy({ vSize.x, GetRowSize(vSize.y) });
	...
	inline void DrawBindInfo(ConfigVar<T>& var, T& val, const std::string& sBind, bool bNewPopup, bool& bLastHovered)
	{
		...
		FDropdown("Type", &tBind.m_iType, { "Key", "Class", "Weapon type", "Item slot", "Command", "None" }, {}, FDropdownEnum::Left, 0, "None", &bHovered);
		bLastHovered = bLastHovered || bHovered;
		...
		case BindEnum::Command:
			tBind.m_iInfo = std::clamp(tBind.m_iInfo, 0, 1);
			FDropdown("Mode", &tBind.m_iInfo, { "Execute", "Toggle" }, {}, FDropdownEnum::Right);
			break;
		case BindEnum::None:
			// No options
			break;
		...
		//if (tBind.m_iType == BindEnum::Key || tBind.m_iType == BindEnum::Command)
		//	FKeybind("Key", tBind.m_iKey);
