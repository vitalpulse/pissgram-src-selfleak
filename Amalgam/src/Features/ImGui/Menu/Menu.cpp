...
#include "../../Amalgam/src/Features/Aimbot/AimbotProjectile/AimbotProjectile.h"
#include "../../BanChecker/BanChecker.h"
#include "snow.h"
...
void CMenu::DrawMenu()
{
	...
	if (Begin("Main", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground))
	{
		...
		ImU32 colTop = ImGui::ColorConvertFloat4ToU32(F::Render.Background0.Value);
		ImU32 colBottom = ImGui::ColorConvertFloat4ToU32(ImVec4(
			F::Render.Background0.Value.x * 0.8f,
			F::Render.Background0.Value.y * 0.8f,
			F::Render.Background0.Value.z * 0.8f,
			F::Render.Background0.Value.w
		));
		pDrawList->AddRectFilledMultiColor(
			vDrawPos,
			{ vDrawPos.x + H::Draw.Scale(flSideSize), vDrawPos.y + vWindowSize.y },
			colTop, colTop, colBottom, colBottom
		);

		// Accent line on right edge of sidebar
		pDrawList->AddRectFilled(
			{ vDrawPos.x + H::Draw.Scale(flSideSize - 2), vDrawPos.y },
			{ vDrawPos.x + H::Draw.Scale(flSideSize), vDrawPos.y + vWindowSize.y },
			ImGui::ColorConvertFloat4ToU32(ImVec4(
				F::Render.Accent.Value.x,
				F::Render.Accent.Value.y,
				F::Render.Accent.Value.z,
				0.3f
			))
		);
		...
		pDrawList->AddRectFilled(
			{ vDrawPos.x + H::Draw.Scale(flSideSize), vDrawPos.y },
			{ vDrawPos.x + vWindowSize.x, vDrawPos.y + vWindowSize.y },
			ImGui::ColorConvertFloat4ToU32(F::Render.Background1.Value)
		);
		...
		else // title
		{
			...
			// Subtle divider line
			pDrawList->AddRectFilled(
				{ vDrawPos.x, vDrawPos.y + H::Draw.Scale(35) },
				{ vDrawPos.x + H::Draw.Scale(flSideSize - 1), vDrawPos.y + H::Draw.Scale(36) },
				ImGui::ColorConvertFloat4ToU32(ImVec4(
					F::Render.Background2.Value.x,
					F::Render.Background2.Value.y,
					F::Render.Background2.Value.z,
					0.5f
				))
			);
			...
			FText(TruncateText("Jollygram", H::Draw.Scale(flSideSize - 28), F::Render.FontBold).c_str(), 0, F::Render.FontBold);
			PopFont();
			PopStyleColor();
		}
		...
		if (BeginTable("AimbotTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("General"))
				{
					...
					FToggle(Vars::Aimbot::General::AlwaysMelee, FToggleEnum::Right);
					FToggle(Vars::Aimbot::General::OnlyDtAimbot, FToggleEnum::Left);
				...
				if (Section("Backtrack", 8))
				{
					...
					FToggle(Vars::Backtrack::PreferOnShot, FToggleEnum::Left);
			...
			TableNextColumn();
			{
				...
				if (Section("Projectile"))
				{
					...
					//FToggle(Vars::Aimbot::Projectile::UseSplashPointDensity, FToggleEnum::Right);
					//PushTransparent(!Vars::Aimbot::Projectile::UseSplashPointDensity.Value);
					//{
						FSlider(Vars::Aimbot::Projectile::SplashPointDensity);
						FSlider(Vars::Aimbot::Projectile::SplashUpdateRate);
						FSlider(Vars::Aimbot::Projectile::TraceVisibility);
					//}
					//PopTransparent();
					PushTransparent(!Vars::Aimbot::Projectile::EdgeDetection.Value);
					{
						FSlider(Vars::Aimbot::Projectile::EdgeSearchRadius, FSliderEnum::Left);
						FSlider(Vars::Aimbot::Projectile::EdgeSamples, FSliderEnum::Right);
						FSlider(Vars::Aimbot::Projectile::MinEdgePoints, FSliderEnum::Left);
						FSlider(Vars::Aimbot::Projectile::MaxEdgePoints, FSliderEnum::Right);
					}
					PopTransparent();
					//FDropdown(Vars::Aimbot::Projectile::SplashAdditions, FDropdownEnum::Left);
					FToggle(Vars::Aimbot::Projectile::EdgeDetection, FToggleEnum::Left);
					//FToggle(Vars::Aimbot::Projectile::Testsplash, FToggleEnum::Left);
					//FToggle(Vars::Aimbot::Projectile::UsePointSplash, FToggleEnum::Right);
					FToggle(Vars::Aimbot::Projectile::UseEyeTrace, FToggleEnum::Right);
					//FToggle(Vars::Aimbot::Projectile::UseHighArc, FToggleEnum::Right);
					//FToggle(Vars::Aimbot::Projectile::AutodetAccountPing, FToggleEnum::Right);
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section("##Debug Projectile"))
					{
						...
						//FSlider(Vars::Aimbot::Projectile::SplashNthRoot, FSliderEnum::Left);
						...
						//FSlider(Vars::Aimbot::Projectile::splashrayoffset, FSliderEnum::Left);
						//FSlider(Vars::Aimbot::Projectile::splashrayinside, FSliderEnum::Right);
						//FSlider(Vars::Aimbot::Projectile::SplashPointsDirect, FSliderEnum::Left);
					} EndSection();
				}
				if (Section("Melee", 8))
	...
	case 2:
	{
		if (BeginTable("Debugtable", 1))
		{
			...
			TableNextColumn();
			{
				...
				{
//					FToggle(Vars::Aimbot::DebugProj::debugsplash, FToggleEnum::Left);
//					FToggle(Vars::Aimbot::DebugProj::airfix, FToggleEnum::Right);
//					FDropdown(Vars::Aimbot::DebugProj::EdgeSplashPriority, FDropdownEnum::Left);
//					FToggle(Vars::Aimbot::DebugProj::TestEdge, FToggleEnum::Right);
//
//					FToggle(Vars::Aimbot::DebugProj::debuglogs, FToggleEnum::Left);
//					FToggle(Vars::Aimbot::DebugProj::debuglogs2, FToggleEnum::Right);
//					//FToggle(Vars::Aimbot::DebugProj::bEnableSplashDelay, FToggleEnum::Left);
//					//FSlider(Vars::Aimbot::DebugProj::kMinSplashQuality, FSliderEnum::Right);
//					//FSlider(Vars::Aimbot::DebugProj::kMaxSplashDelay, FSliderEnum::Left);
//					//FSlider(Vars::Aimbot::DebugProj::kSplashProximity, FSliderEnum::Right);
///*					FToggle(Vars::Aimbot::DebugProj::RollExploit, FToggleEnum::Right);
//					FSlider(Vars::Aimbot::DebugProj::Step, FSliderEnum::Left)*/;
//
//					//FDropdown(Vars::Aimbot::DebugProj::SplashMode, FDropdownEnum::Left, 0, &Hovered); FTooltip("Pissgram is made by cowboy. Amalgam is default amalgam's", Hovered);
//					//FDropdown(Vars::Aimbot::DebugProj::RocketSplashMode, FDropdownEnum::Right, 0, &Hovered); FTooltip("Special splash type for rockets, more expensive", Hovered);
//					//FSlider(Vars::Aimbot::DebugProj::SplashPointsDirect, FSliderEnum::Left);
//					//FSlider(Vars::Aimbot::DebugProj::SplashPointsArc, FSliderEnum::Right);
//					FSlider(Vars::Aimbot::DebugProj::bleedstrictness, FSliderEnum::Left);
//					FSlider(Vars::Aimbot::DebugProj::explosionoffset, FSliderEnum::Right);
//					//FToggle(Vars::Aimbot::DebugProj::HAHAHAHAHHAHA, FToggleEnum::Right);
//					FDropdown(Vars::Aimbot::DebugProj::debug_opt, FDropdownEnum::Left);
//					FSlider(Vars::Aimbot::DebugProj::los_checks, FSliderEnum::Right);
//					FSlider(Vars::Aimbot::DebugProj::max_points, FSliderEnum::Left);
					FSlider(Vars::Aimbot::DebugProj::fick_freq);
					//FSlider(Vars::Aimbot::DebugProj::flick_dist);
					FSlider(Vars::Aimbot::DebugProj::strafe_strictness);
				} EndSection();
			}
			EndTable();
		}
			...
			&& BeginTable("VisualsESPTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Colors", 8))
				{
					...
					FColorPicker(Vars::Colors::ElementsColor, FColorPickerEnum::Right);
					FColorPicker(Vars::Colors::FullHPColor, FColorPickerEnum::Left);
					FColorPicker(Vars::Colors::ElementsOutline, FColorPickerEnum::Right);
					FColorPicker(Vars::Colors::OverhealColor, FColorPickerEnum::Left);
					FColorPicker(Vars::Colors::MidHPColor, FColorPickerEnum::Right);
					FColorPicker(Vars::Colors::UberColor, FColorPickerEnum::Left);
					FColorPicker(Vars::Colors::LowHPColor, FColorPickerEnum::Right);
			...
			TableNextColumn();
			{
				...
				{
					FDropdown("Extra ESP Options", &tGroup.m_iESPOptions, { "Name use Group Color", "Dynamic Health Bars", "Display Tags at Bottom", "Tags override color", "Centered Health Text" }, {}, FDropdownEnum::Multi);
				...
				if (Section("Misc", 8))
				{
					...
					{
						FSlider("Offset", &tGroup.m_iOffscreenArrowsOffset, 150, 600, 5, "%i", FSliderEnum::Precision);
						FSlider("Max distance", &tGroup.m_flOffscreenArrowsMaxDistance, 600.f, 1500.f, 50.f, "%g", FSliderEnum::Min | FSliderEnum::Precision);
						FSlider("Min Size", &tGroup.m_flOffscreenArrowsMinSize, 15.f, 30.f, 1.f, "%g", FSliderEnum::Min | FSliderEnum::Precision);
						FSlider("Max Size", &tGroup.m_flOffscreenArrowsMaxSize, 30.f, 75.f, 1.f, "%g", FSliderEnum::Min | FSliderEnum::Precision);
						FSlider("Flicker Distance", &tGroup.m_flOffscreenArrowsFlickerDistance, 10.f, 500.f, 10.f, "%g", FSliderEnum::Min | FSliderEnum::Precision);
						EndPopup();
					}
		...
		if (BeginTable("VisualsMiscTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Effects"))
				{
					...
					FToggle(Vars::Visuals::Effects::CritGlow);
				} EndSection();
			}
			/* Column 2 */
			TableNextColumn();
			{
				...
				if (Section("World"))
				{
					...
					FToggle(Vars::Visuals::World::matFullbright, FToggleEnum::Left);
		...
		if (BeginTable("MenuTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Indicators"))
				{
					...
					// Organizing for easier navigation
			...
			TableNextColumn();
			{
				if (Section("Movement"))
				{
					...
					PushTransparent(Vars::Misc::Movement::AutoStrafe.Value != Vars::Misc::Movement::AutoStrafeEnum::Directional);
					{
						...
						FSlider(Vars::Misc::Movement::AutoStrafeSmoothing);
					}
					PopTransparent();
			...
			PushDisabled(F::PlayerUtils.m_bLoad);
			{
				...
				if (FButton("Export", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					...
					if (fStream.is_open())
					{
						...
						SDK::Output("Amalgam", "Copied playerlist to clipboard", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}

				{
					...
					if (FButton("Import", FButtonEnum::Fit | FButtonEnum::SameLine))
					{
						...
						{
							SDK::Output("Amalgam", "Failed to import playerlist", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
						}
					}
					...
					if (FBeginPopupModal("ImportPlayerlist", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
					{
						...
						if (FButton("Import", FButtonEnum::Left))
						{
							...
							F::PlayerUtils.m_bSave = true;
							SDK::Output("Amalgam", "Imported playerlist", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
				...
				if (FButton("Backup", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					try
					{
						...
						SDK::Output("Amalgam", "Saved backup playerlist", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
					catch (...)
					{
						SDK::Output("Amalgam", "Failed to backup playerlist", Vars::Menu::Theme::Accent.Value, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}
			}
			PopDisabled();
...
void CMenu::MenuSettings(int iTab)
{
	...
	case 3:
	{
		...
		if (Section("Extras / QoL"))
		{
			if (FButton("Disable Voice/Text Chat", FButtonEnum::Left))
				I::EngineClient->ClientCmd_Unrestricted("hud_saytext_time 0; voice_enable 0;");
			if (FButton("Enable Voice/Text Chat", FButtonEnum::Right | FButtonEnum::SameLine))
				I::EngineClient->ClientCmd_Unrestricted("hud_saytext_time 12; voice_enable 1;");
		}
		EndSection();

		if (Section("Local Servers"))
		{
			// QoL buttons for local testing
			if (FButton("Infinite Bot Health", FButtonEnum::Left))
				I::EngineClient->ClientCmd_Unrestricted("ent_fire !picker addoutput \"health 999999");
			if (FButton("Remove Dormancy", FButtonEnum::Right | FButtonEnum::SameLine))
				I::EngineClient->ClientCmd_Unrestricted("sv_force_transmit_ents 1");
			if (FButton("Infinite Ammo", FButtonEnum::Left))
				I::EngineClient->ClientCmd_Unrestricted("sv_cheats 1;alias +infiniteammo \"impulse 101; wait 60; +infiniteammo\"; +infiniteammo");
			if (FButton("Remove Round Timer", FButtonEnum::Right | FButtonEnum::SameLine))
				I::EngineClient->ClientCmd_Unrestricted("ent_fire team_round_timer disable");
		} EndSection();
...
{
	const char* sName;
	std::string sType;      // The behavior type (hold/toggle/double/class/weapon/slot)
	std::string sKey;       // The key name or trigger info
	std::string sState;     // On/Off status
	std::string sValue;     // For displaying slider values

	int iBind;
	Bind_t& tBind;
};

// Helper function to get the primary value string for a bind
std::string GetBindValueString(Bind_t& tBind, int iBind)
{
	// If bind has no modified vars, return empty
	if (tBind.m_vVars.empty())
		return "";

	// Get the first modified variable's value - only for sliders (int/float without dropdown values)
	for (auto pVar : tBind.m_vVars)
	{
		if (auto pVarInt = pVar->As<int>())
		{
			if (pVarInt->Map.contains(iBind))
			{
				// Only show value for sliders (no dropdown values)
				if (pVarInt->m_vValues.empty())
					return std::to_string(pVarInt->Map[iBind]);
			}
		}
		else if (auto pVarFloat = pVar->As<float>())
		{
			if (pVarFloat->Map.contains(iBind))
			{
				float val = pVarFloat->Map[iBind];
...
void CMenu::DrawBinds()
{
	...
	std::function<void(int)> getBinds = [&](int iParent)
		{
			for (int iBind = 0; iBind < F::Binds.m_vBinds.size(); iBind++)
			{
				...
				if (tBind.m_iVisibility == BindVisibilityEnum::Always ||
					tBind.m_iVisibility == BindVisibilityEnum::WhileActive && tBind.m_bActive ||
					m_bIsOpen)
				{
					std::string sType, sKey, sState, sValue;
					...
					// Get the value from modified variables
					sValue = GetBindValueString(tBind, iBind);

					// Only show state if there's no value to display
					if (sValue.empty())
						sState = tBind.m_bActive ? "On" : "Off";
					else
						sState = ""; // Don't show state if we're showing a value

					vInfo.emplace_back(tBind.m_sName.c_str(), sType, sKey, sState, sValue, iBind, tBind);
				}
	...
	float flTypeWidth = 0, flNameWidth = 0, flKeyWidth = 0, flStateValueWidth = 0;
	PushFont(F::Render.FontSmall);
	for (auto& [sName, sType, sKey, sState, sValue, iBind, tBind] : vInfo)
	{
		...
		// Use whichever is displayed - state or value
		if (!sState.empty())
			flStateValueWidth = std::max(flStateValueWidth, FCalcTextSize(sState.c_str()).x);
		else if (!sValue.empty())
			flStateValueWidth = std::max(flStateValueWidth, FCalcTextSize(sValue.c_str()).x);
	}
	PopFont();
	...
	flKeyWidth += H::Draw.Scale(9);
	flStateValueWidth += H::Draw.Scale(9);

	float flWidth = flTypeWidth + flNameWidth + flKeyWidth + flStateValueWidth + (m_bIsOpen ? H::Draw.Scale(113) : H::Draw.Scale(14));
	...
	if (Begin("Binds", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing))
	{
		...
		if (Vars::Menu::BindWindowTitle.Value)
		{
			ImU32 uMiddle = ImGui::GetColorU32((const ImVec4)F::Render.Accent);
			ImVec2 vMin = { vWindowPos.x + 2.f, vWindowPos.y + H::Draw.Scale(26) + 1.0f };
			ImVec2 vMax = { vWindowPos.x + flWidth - H::Draw.Scale(2), vWindowPos.y + H::Draw.Scale(27) + 1.0f };

			SetCursorPos({ H::Draw.Scale(11), H::Draw.Scale(7) });
			PushFont(F::Render.FontRegular);
			FText("Binds");
			PopFont();

			GetWindowDrawList()->AddRectFilledMultiColor(vMin, vMax, uMiddle, uMiddle, uMiddle, uMiddle);
			iListStart = 36;
		}
		...
		for (auto& [sName, sType, sKey, sState, sValue, iBind, tBind] : vInfo)
		{
			float flPosX = H::Draw.Scale(12);
			float flPosY = H::Draw.Scale(iListStart + 18 * i);

			// Draw type (behavior)
			SetCursorPos({ flPosX, flPosY });
			...
			ImVec2 vTextPos = GetDrawPos() + GetCursorPos();
			ImVec2 vTextSize = FCalcTextSize(sName);
			FText(sName);

			// Draw strikethrough if disabled
			if (!tBind.m_bEnabled)
			{
				float flLineY = vTextPos.y + vTextSize.y * 0.5f;
				ImU32 uLineColor = ImGui::GetColorU32(tBind.m_bActive ? F::Render.Active.Value : F::Render.Inactive.Value);
				GetWindowDrawList()->AddLine(
					{ vTextPos.x, flLineY },
					{ vTextPos.x + vTextSize.x, flLineY },
					uLineColor,
					H::Draw.Scale(1.0f)
				);
			}
			...
			{
				// Draw the value instead
				SetCursorPos({ flPosX, flPosY });
				PushStyleColor(ImGuiCol_Text, tBind.m_bActive ? F::Render.Accent.Value : F::Render.Inactive.Value);
				FText(sValue.c_str());
				PopStyleColor();
			}
			flPosX += flStateValueWidth;
...
void CMenu::Render()
{
	...
	if (!F::Snow.IsInitialized())
		F::Snow.Initialize(150); // 150 snowflakes

	if (m_bIsOpen)
	{
		F::Snow.Update(ImGui::GetIO().DisplaySize);
		F::Snow.Render();
