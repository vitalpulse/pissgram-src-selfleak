...
//	}
//
//	const auto& cColor = m_bSynced ? Vars::Menu::Theme::IndicatorsColor.Value : Vars::Menu::Theme::Background.Value;
//
...
//	H::Draw.StringOutlined(fFont, x, y += nTall, cColor, Vars::Menu::Theme::Background.Value, align, std::format("Step Size: {}", m_flMantissaStep).c_str());
...
{
	static auto tf_use_fixed_weaponspreads = H::ConVars.FindVar("tf_use_fixed_weaponspreads");
	if (tf_use_fixed_weaponspreads)
	{
		bool bFixedSpread = tf_use_fixed_weaponspreads->GetBool();
		const auto pMatchDesc = I::TFGameRules()->GetMatchGroupDescription();
		if (pMatchDesc)
...
void CNoSpreadHitscan::Draw(CTFPlayer* pLocal)
{
	...
	const auto& cColor = m_bSynced ? Vars::Menu::Theme::IndicatorsColor.Value : Vars::Menu::Theme::Inactive.Value;
